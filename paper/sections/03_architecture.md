...
|-----------|---------------|-------|------|
| SNN | snntorch (Eshraghian et al., 2023) | 4,116 LIF neurons, 10 timesteps | Conscious temporal dynamics: spike timing, synchrony, STDP learning |
...
We emphasize the scale gap: biological consciousness involves approximately 86 billion neurons. Our combined substrate count of ~140,000 is six orders of magnitude smaller. Whether consciousness requires a particular neural scale is an open question; our framework makes no assumptions either way.
...
**Phase I — Competition and selection (GWT).** Candidate signals arrive from neural substrates, memory, and external input. Each carries a salience score composed of bottom-up activation (40%), top-down goal relevance (30%), emotional weight (20%), and recency (10%). An attention bottleneck ranks candidates and enforces a capacity limit of 7±2 items, consistent with GWT's limited-capacity workspace. Winners that sustain activation above threshold undergo nonlinear amplification — the *ignition* event that marks the transition to conscious access (Dehaene & Naccache, 2001). Ignited content is broadcast to all downstream modules.
