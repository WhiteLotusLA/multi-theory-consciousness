...
Within each category, sub-factors are blended: bottom-up combines raw activation strength (60%) with novelty (40%), and top-down combines goal relevance (50%) with task relevance (50%). The formula above represents the category-level weights; the full computation includes these sub-weights and an additive priority boost for safety-critical content.
...
An `AttentionBottleneck` enforces a capacity limit inspired by Miller's (1956) "magical number seven, plus or minus two" — the well-established finding that human short-term memory holds roughly 5 to 9 items simultaneously. The bottleneck ranks all candidates by salience and admits only the top 5 to 9 (the default configuration uses a fixed capacity of 7), discarding the rest to an unconscious buffer. This is a simplification on two fronts: GWT describes a capacity limit but does not specify a fixed number, and the relationship between Miller's short-term memory findings and Baars' workspace capacity is assumed rather than established.
...
The `Protoself` maps homeostatic drives and optional system metrics to a five-dimensional `BodyState` vector: metabolic state, energy level, pain signals, circadian phase, and temperature. A sixth field, interoceptive state, stores per-drive detail as a dictionary but is not included in the numerical vector. From this vector, it derives three *primordial feelings* — pleasure/pain, vitality, and arousal — computed from body state *trends* rather than snapshots. A body that is becoming more satisfied produces positive valence; a body in decline produces negative valence, regardless of current absolute state.
...
| Damasio | Five-dimensional body state (plus dictionary field) | Reduces biological homeostasis to a low-dimensional proxy |
